...
from nnunet.training.network_training.nnUNet_variants.loss_function.nnUNetTrainerV2_Loss_DiceCE_noSmooth import nnUNetTrainerV2_Loss_DiceCE_noSmooth
...
class nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads(nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmup):
    """
    ...
    def maybe_update_lr(self, epoch=None):
        if self.epoch < self.warmup_duration:
            ...
            if epoch is not None:
                ...
                ep = self.epoch - (self.warmup_duration - 1)
            assert ep > 0, "epoch must be >0"
            self.optimizer.param_groups[0]['lr'] = poly_lr(ep, self.max_num_epochs - self.warmup_duration, self.initial_lr, 0.9)
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        if train:
            ...
            if self.epoch > 49: self.initialize_optimizer_and_scheduler(seg_heads_only=False)
...
class nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads_warmupNetworkTrain(nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
        self.num_epochs_sgd_warmup = 30
        self.warmup_max_lr = 4e-3
...
class nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads_warmupNetworkTrain_1En3(nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads_warmupNetworkTrain):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
...
class nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads_warmupNetworkTrain_oldschedule(
    nnUNetTrainerV2_Loss_DiceCE_noSmooth_warmupSegHeads_warmupNetworkTrain):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True,
                 stage=None,
                 unpack_data=True, deterministic=True, fp16=False, fim_file=None):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
...
class nnUNetTrainerV2_warmupSegHeads(nnUNetTrainerV2_warmup):
    """
    There are certainly bugs hidden in here :-) It kinda works though
    """
    ...
    def maybe_update_lr(self, epoch=None):
        if self.epoch < self.warmup_duration:
            ...
            if epoch is not None:
                ...
                ep = self.epoch - (self.warmup_duration - 1)
            assert ep > 0, "epoch must be >0"
            self.optimizer.param_groups[0]['lr'] = poly_lr(ep, self.max_num_epochs - self.warmup_duration,
                                                           self.initial_lr, 0.9)
            self.print_to_log_file("lr was set to:", np.round(self.optimizer.param_groups[0]['lr'], decimals=6))
...
class nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain(nnUNetTrainerV2_warmupSegHeads):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
...
class nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain_1En3(nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
...
class nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain_oldschedule_bfnnunet(
    nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True,
                 stage=None,
                 unpack_data=True, deterministic=True, fp16=False, fim_file=None):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
...
class nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain_oldschedule_modelgenesis(
    nnUNetTrainerV2_warmupSegHeads_warmupNetworkTrain):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True,
                 stage=None,
                 unpack_data=True, deterministic=True, fp16=False, fim_file=None):
        ...
        self.num_epochs_sgd_warmup = 50
        self.warmup_max_lr = 4e-3
        self.warmup_duration = 0  # this is for the seg heads
        self.max_num_epochs = 1000
