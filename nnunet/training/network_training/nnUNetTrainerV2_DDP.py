...
class nnUNetTrainerV2_DDP(nnUNetTrainerV2):
    def __init__(self, plans_file, fold, local_rank, output_folder=None, dataset_directory=None, batch_dice=True,
        ...
        dist.init_process_group(backend='nccl', init_method='env://',timeout=datetime.timedelta(seconds=7000))
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
                                                         ...
                                                         verbose: bool = True, mixed_precision=True, region_vec=None) -> Tuple[
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        for k, value in checkpoint['state_dict'].items():
            ...
            if key not in curr_state_dict_keys:
                ...
                # STUPID BUGFIX ARRRRG
                if key == 'module.decoder.segmentation_output.weight':
                    key = "module.decoder.deep_supervision_outputs.4.weight"
                elif key == "module.decoder.segmentation_output.bias":
                    key = "module.decoder.deep_supervision_outputs.4.bias"
                else:
                    key = key[7:]
