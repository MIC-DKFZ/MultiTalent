...
from nnunet.dataset_conversion.Task100_MultiTalent import MultiTalent_regions, MultiTalent_region_output_idx_mapping, \
    MultiTalent_regions_class_order, MultiTalent_valid_regions
...
class MultiTalent_trainer_ddp(nnUNetTrainerV2_DDP):
    def __init__(self, plans_file, fold, local_rank, output_folder=None, dataset_directory=None, batch_dice=True,
        ...
        batch_dice = True
        super().__init__(plans_file=plans_file, fold=fold, local_rank=local_rank, output_folder=output_folder,
    ...
    def process_plans(self, plans):
        ...
        #self.patch_size = np.array((48,96,96)).astype(int)
    ...
    def finish_online_evaluation(self):
        ...
        #print(self.online_eval_fn,self.online_eval_fp,self.online_eval_tp)

        global_dc_per_class = [l for l in [2 * i / (np.clip(2 * i + j + k, a_min=1e-8, a_max=None))
                                           for i, j, k in zip(self.online_eval_tp, self.online_eval_fp,
                                                              self.online_eval_fn)] if not np.isnan(l).any()]
            ...
            if not isfile(join(self.dataset_directory, "splits_custom.pkl")):
                ...
                for task_id in np.unique([int(i.split("_")[0]) for i in keys]):
                    if task_id != 46:
                        ...
                        remaining_task_46_ids = [i for i in keys if i.startswith("046_PAN")]  # this is just the new task46 cases
    ...
    def get_basic_generators(self):
        ...
        num_cases_per_dataset = [len([i for i in keys if i.startswith(j + '_')]) for j in dataset_dentifiers]
        probabilities = np.array([1 / (num_cases_per_dataset[dataset_dentifiers.index(i.split('_')[0])]**0.5) for i in keys])
        ...
        num_cases_per_dataset = [len([i for i in keys_val if i.startswith(j + '_')]) for j in dataset_dentifiers_val]
        probabilities_val = np.array([1 / (num_cases_per_dataset[dataset_dentifiers_val.index(i.split('_')[0])]**0.5) for i in keys_val])
...
class MultiTalent_trainer_ddp_2000ep(MultiTalent_trainer_ddp):
    def __init__(self, plans_file, fold, local_rank, output_folder=None, dataset_directory=None, batch_dice=True,
                 stage=None, unpack_data=True, deterministic=True, distribute_batch_size=False, fp16=False):
        batch_dice = True
        super().__init__(plans_file=plans_file, fold=fold, local_rank=local_rank, output_folder=output_folder,
