...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True, region_vec = None) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          ...
                                          verbose: bool, region_vec=None) -> Tuple[np.ndarray, np.ndarray]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        ...
                        gaussian_importance_map, region_vec=region_vec)[0]
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
                                    ...
                                    verbose: bool = True, region_vec = None) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           ...
                                           mult: np.ndarray or torch.tensor = None, region_vec = None) -> torch.tensor:
        ...
        for m in range(mirror_idx):
            if m == 0:
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(x, region_vec))
                else:
            ...
            if m == 1 and (2 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, )), region_vec))
                else:
            ...
            if m == 2 and (1 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (3,)), region_vec))
                else:
            ...
            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3)), region_vec))
                else:
            ...
            if m == 4 and (0 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (2,)), region_vec))
                else:
            ...
            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2)), region_vec))
                else:
            ...
            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2)), region_vec))
                else:
            ...
            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                if region_vec is not None:
                    pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2)), region_vec))
                else:
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                predicted_patch = self._internal_maybe_mirror_and_pred_2D(
                    ...
                    gaussian_importance_map, region_vec)[0]
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
                                    ...
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    all_in_gpu: bool = False, verbose: bool = True, regio_vec = None) -> Tuple[np.ndarray, np.ndarray]:
        if all_in_gpu:
            raise NotImplementedError
        ...
        for s in range(x.shape[1]):
            pred_seg, softmax_pres = self._internal_predict_2D_2Dconv(
                x[:, s], min_size, do_mirroring, mirror_axes, regions_class_order, pad_border_mode, pad_kwargs, verbose, regio_vec)
    ...
    def predict_3D_pseudo3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool,
                                   ...
                                   pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                   verbose: bool = True, regio_vec = None) -> Tuple[np.ndarray, np.ndarray]:
        if all_in_gpu:
            raise NotImplementedError
        assert len(x.shape) == 4, "data must be c, x, y, z"
        ...
        for s in range(extra_slices, data.shape[1] - extra_slices):
            ...
            pred_seg, softmax_pres = \
                self._internal_predict_2D_2Dconv(d, min_size, do_mirroring, mirror_axes,
                                                 regions_class_order, pad_border_mode, pad_kwargs, verbose, regio_vec = None)
            predicted_segmentation.append(pred_seg[None])
            softmax_pred.append(softmax_pres[None])
        predicted_segmentation = np.vstack(predicted_segmentation)
        softmax_pred = np.vstack(softmax_pred).transpose((1, 0, 2, 3))
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred_seg, softmax_pres = self._internal_predict_2D_2Dconv_tiled(
                ...
                pad_border_mode, pad_kwargs, all_in_gpu, verbose, region_vec = None)
