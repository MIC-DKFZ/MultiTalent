...
import requests
from batchgenerators.utilities.file_and_folder_operations import join, isfile, isdir, maybe_mkdir_p, load_pickle, write_pickle
...
def get_available_models():
    available_models = {
       ...
       "Task135_KiTS2021": {
        ...
        "Task100_MultiTalent": {
            'description': "Pretrained models for the paper: MultiTalent: A Multi-Dataset Approach to Medical Image Segmentation\n"
                           "Infos about used datasets can be found in that publication  \n"
                           "While all datasets are public, some of them might not allow comercial usage. ",
            'url': (
                "https://zenodo.org/record/8297767/files/Task100_MultiTalent.zip?download=1",
            )
        },
    }
...
def download_and_install_pretrained_model_by_name(taskname):
    ...
    if taskname == 'Task100_MultiTalent':
        for file in os.listdir(join(network_training_output_dir, taskname)):
        ...
        for config in os.listdir(join(network_training_output_dir, "3d_fullres", taskname)):
            for fold in os.listdir(join(network_training_output_dir, "3d_fullres", taskname, config)):
                if fold != 'plans.pkl':
                    model_pkl = load_pickle(join(network_training_output_dir, "3d_fullres", taskname, config, fold,
                                                 'model_final_checkpoint.model.pkl'))

                    model_pkl['name'] = config.split('__')[0]
                    write_pickle(model_pkl, join(network_training_output_dir, "3d_fullres", taskname, config, fold,
                                                 'model_final_checkpoint.model.pkl'))


        shutil.rmtree(join(network_training_output_dir, taskname))
