...
from multitalent.evaluation.evaluate_predictions import load_summary_json
from multitalent.paths import nnUNet_results
from multitalent.utilities.dataset_name_id_conversion import maybe_convert_to_dataset_name, convert_dataset_name_to_id
from multitalent.utilities.file_path_utilities import get_output_folder
...
if __name__ == '__main__':
    use_these_trainers = {
        'nnUNetTrainerDiceCELoss_noSmooth': ('nnUNetPlans', ),  # my trainer
        'nnUNetTrainer_MoreDiverseSampling_noSmooth': ('nnUNetPlans', ),  # my trainer
        'nnUNetTrainer': ('nnUNetPlans', 'nnUNetPlans_resencUNet'),  # my trainer
    }

    configs = ("3d_fullres", "3d_fullres_SD", "3d_fullres_192x192x192", "3d_fullres_resenc", "3d_fullres_resenc_bs12", "3d_fullres_resenc_bs24", "3d_fullres_resenc_bs48", "3d_fullres_resenc_bs80")
    all_results_file= join(nnUNet_results, '2023_autopet_results.csv')
    datasets = [221, 222]
    collect_results(use_these_trainers, datasets, all_results_file, configs)

    folds = (0, 1, 2, 3, 4)
    output_file = join(nnUNet_results, '2023_autopet_summary.csv')
