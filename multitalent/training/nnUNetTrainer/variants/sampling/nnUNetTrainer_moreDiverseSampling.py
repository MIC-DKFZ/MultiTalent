...
from multitalent.training.loss.compound_losses import DC_and_BCE_loss, DC_and_CE_loss
...
class nnUNetDataLoaderMoreDiverseSampling(nnUNetDataLoaderBase):
    def get_bbox(self, data_shape: np.ndarray, force_fg: bool, class_locations: Union[dict, None],
        ...
        if not force_fg and not self.has_ignore:
            ...
            if voxels_of_that_class is not None:
                ...
                #################################################################
                if self.has_ignore or force_fg:
                    # # random offset for selected voxel
                    # orig = deepcopy(selected_voxel)
                    # AS OPPOSED TO BETTER IGN SAMPLING WE ONLY DO // 3 HERE INSTEAD OF // 2
                    allowed_max_neg_offset = [min(s, p // 3) for s, p in zip(selected_voxel[1:], self.patch_size)]
                    allowed_max_pos_offset = [min(d - s, p // 3) for s, p, d in
...
class nnUNetDataLoader2DMoreDiverseSampling(nnUNetDataLoader2D):
    def get_bbox(self, data_shape: np.ndarray, force_fg: bool, class_locations: Union[dict, None],
                 overwrite_class: Union[int, Tuple[int, ...]] = None, verbose: bool = False):
        return nnUNetDataLoaderMoreDiverseSampling.get_bbox(self, data_shape, force_fg, class_locations,
                                                              overwrite_class, verbose)


# the following class is evil!
class nnUNetDataLoader3DMoreDiverseSampling(nnUNetDataLoader3D):
    def get_bbox(self, data_shape: np.ndarray, force_fg: bool, class_locations: Union[dict, None],
                 overwrite_class: Union[int, Tuple[int, ...]] = None, verbose: bool = False):
        return nnUNetDataLoaderMoreDiverseSampling.get_bbox(self, data_shape, force_fg, class_locations,
                                                              overwrite_class, verbose)
...
class nnUNetTrainer_MoreDiverseSampling(nnUNetTrainer):
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        dataset_tr, dataset_val = self.get_tr_and_val_datasets()

        if dim == 2:
            dl_tr = nnUNetDataLoader2DMoreDiverseSampling(dataset_tr,
                                                        self.batch_size,
                                                        initial_patch_size,
            ...
            dl_val = nnUNetDataLoader2DMoreDiverseSampling(dataset_val,
                                                         self.batch_size,
                                                         self.configuration_manager.patch_size,
                                                         self.configuration_manager.patch_size,
                                                         self.label_manager,
            ...
            dl_tr = nnUNetDataLoader3DMoreDiverseSampling(dataset_tr,
                                                        self.batch_size,
                                                        initial_patch_size,
                                                        self.configuration_manager.patch_size,
                                                        self.label_manager,
                                                        oversample_foreground_percent=self.oversample_foreground_percent,
                                                        sampling_probabilities=None, pad_sides=None)
            dl_val = nnUNetDataLoader3DMoreDiverseSampling(dataset_val,
                                                         self.batch_size,
                                                         self.configuration_manager.patch_size,
                                                         self.configuration_manager.patch_size,
                                                         self.label_manager,
...
class nnUNetTrainer_MoreDiverseSampling_noSmooth(nnUNetTrainer_MoreDiverseSampling):
