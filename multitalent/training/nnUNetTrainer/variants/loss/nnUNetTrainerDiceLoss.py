...
import numpy as np
import torch
from multitalent.training.nnUNetTrainer.variants.data_augmentation.nnUNetTrainerNoMirroring import \
...
from multitalent.training.loss.dice import SoftDiceLoss, get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss
from multitalent.utilities.ddp_allgather import AllGatherGrad
...
class nnUNetTrainerDiceCELoss_smooth1en8(nnUNetTrainer):
    def _build_loss(self):
        # set smooth to 0
        if self.label_manager.has_regions:
            loss = DC_and_BCE_loss({},
                                   {'batch_dice': self.configuration_manager.batch_dice,
                                    'do_bg': True, 'smooth': 1e-8, 'ddp': self.is_ddp},
                                   use_ignore_label=self.label_manager.ignore_label is not None,
                                   dice_class=MemoryEfficientSoftDiceLoss)
        else:
            loss = DC_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                   'smooth': 1e-8, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1, weight_dice=1,
                                  ignore_label=self.label_manager.ignore_label,
                                  dice_class=MemoryEfficientSoftDiceLoss)
