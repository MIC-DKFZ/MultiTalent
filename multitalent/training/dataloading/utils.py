    ...
    try:
        ...
        if overwrite_existing or not isfile(data_npy):
            try:
                ...
                print(f"Unable to open preprocessed file {npz_file}. Rerun multitalent_preprocess!")
                raise e
            np.save(data_npy, npz_content['data'].astype(np.float16)) ####unpack as float16!
        ...
        if verify_npy:
            try:
                ...
                if fail_ctr < 2:
                    ...
                    raise RuntimeError("Unable to fix unpacking. Please check your system or rerun multitalent_preprocess")
