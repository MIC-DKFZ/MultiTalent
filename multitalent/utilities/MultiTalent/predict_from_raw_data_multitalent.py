...
class nnUNetPredictor_MT(object):
    def __init__(self,
        ...
        self.id = [target_dataset_id]

    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
                                             use_folds: Union[Tuple[Union[int, str]], None],
                                             checkpoint_name: str = 'checkpoint_final.pth',
                                             input_channels: dict|int = None,
                                             output_channels: dict = None):
        """
        This is used when making predictions with a trained model
        """
        if use_folds is None:
            use_folds = nnUNetPredictor_MT.auto_detect_available_folds(model_training_output_dir, checkpoint_name)

        dataset_json = load_json(join(model_training_output_dir, self.id[0] + '_dataset.json'))
        plans = load_json(join(model_training_output_dir, self.id[0] + '_plans.json'))
        print(plans, join(model_training_output_dir, self.id[0] + '_plans.json'))
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prediction = self.network([x], self.id)[0] #list is returned...
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction += torch.flip(self.network([torch.flip(x, (*axes,))], self.id)[0], (*axes,))
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('-o', type=str, required=True,
                        help='Output f older. If it does not exist it will be created. Predicted segmentations will '
    ...
    all_num_seg_heads = {}
    all_num_stems = {}
    for files in os.listdir(args.m):
        if files.endswith('plans.json'):
            all_ids.append(files[:-11])
            plan = load_json(join(args.m, files))
            all_num_stems[files[:-11]] = len(plan["configurations"]["3d_fullres"]["normalization_schemes"])
            dataset_json = load_json(join(args.m, files[:-11]+ '_dataset.json'))
            all_num_seg_heads[files[:-11]] = len(dataset_json['labels'].keys())
            if 'ignore' in dataset_json['labels'].keys():
                all_num_seg_heads[files[:-11]] = all_num_seg_heads[files[:-11]] -1

    if args.target_id is not None:
        all_ids = [f"{int(i):03d}" for i in args.target_id]
