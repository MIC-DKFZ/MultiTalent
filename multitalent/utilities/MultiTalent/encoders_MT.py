    ...
    def __init__(self,
        ...
        for s in range(n_stages):
            ...
            if pool == 'max' or pool == 'avg':
                ...
                raise RuntimeError()
            if s == 0 and isinstance(input_channels, dict):
                stem = {}
                for id in input_channels.keys():
                    stem[id] = StackedConvBlocks(1, conv_op, input_channels[id], features_per_stage[s],
                                                                 kernel_sizes[s], conv_stride, conv_bias, norm_op,
                                                                 norm_op_kwargs, dropout_op, dropout_op_kwargs,
                                                                 nonlin, nonlin_kwargs)
                self.stem = nn.ModuleDict(stem)

                if isinstance(features_per_stage[s], (tuple, list)):
                    input_channels = features_per_stage[s][0]
                    tmp = features_per_stage[s][1:]
                else:
                    input_channels = features_per_stage[s]
                    tmp = features_per_stage[s]

                stage_modules.append(StackedConvBlocks(
                    n_conv_per_stage[s]-1, conv_op, input_channels, tmp, kernel_sizes[s], 1,
        ...
        if self.stem is not None:
            ...
            for b in x:
                if x_nostem is None:
                    ...
                    x_nostem = torch.cat((x_nostem, b), dim=0)
            x = x_nostem
...
class ResidualEncoder(nn.Module):
    def __init__(self,
        ...
        if not disable_default_stem:
            ...
            if isinstance(input_channels,  (int, np.integer)):
                self.stem = StackedConvBlocks(1, conv_op, input_channels, stem_channels, kernel_sizes[0], 1, conv_bias,
                                              norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs)
            elif isinstance(input_channels, dict):
                stem = {}
                for id in input_channels.keys():
                    stem[id] = StackedConvBlocks(1, conv_op, input_channels[id], stem_channels,
                                                                       kernel_sizes[0], 1, conv_bias, norm_op,
