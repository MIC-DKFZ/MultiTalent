...
from multitalent.experiment_planning.dataset_fingerprint.fingerprint_extractor import DatasetFingerprintExtractor
from multitalent.experiment_planning.experiment_planners.default_experiment_planner import ExperimentPlanner
from multitalent.experiment_planning.verify_dataset_integrity import verify_dataset_integrity
...
def extract_fingerprints(dataset_ids: List[int], fingerprint_extractor_class_name: str = 'DatasetFingerprintExtractor',
                         ...
                         clean: bool = True, verbose: bool = True):
    """
    clean = False will not actually run this. This is just a switch for use with multitalent_plan_and_preprocess where
    we don't want to rerun fingerprint extraction every time.
    """
    fingerprint_extractor_class = recursive_find_python_class(join(multitalent.__path__[0], "experiment_planning"),
...
def preprocess_dataset(dataset_id: int,
    ...
    for n, c in zip(num_processes, configurations):
        ...
        print(configuration_manager.preprocessor_class)
